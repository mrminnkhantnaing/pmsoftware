...
@section('title')
    {{ $building->name }} | Dashboard | {{ config('app.name', 'PM Software') }}
@endsection
...
@section('content')
    ...
    <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
        <div class="ps-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 p-0">
                    ...
                    <li class="breadcrumb-item">
                        <a class="text-dark" href="{{ route('dashboard.buildingsIndex') }}">BUILDINGS</a>
                    </li>
        ...
        <div class="ms-auto">
            <a class="btn btn-outline-primary" href="{{ route('dashboard.buildingsIndex') }}">BACK</a>
        </div>
    </div>
                            ...
                            <table class="table mb-0">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($building->floors as $floor)
                                        <tr>
                                            <th scope="row">{{ $i }}</th>
                                            <td><a class="text-dark" title="View" href="{{ route('dashboard.floorsShow', $floor->id) }}">{{ $floor->name }}</a></td>
                                            <td>{{ $floor->flats->count() }}</td>
                                            <td>{{ $floor->partitions->count() }}</td>
                                            <td>
                                                <a class="btn btn-outline-dark" title="View" href="{{ route('dashboard.floorsShow', $floor->id) }}">
