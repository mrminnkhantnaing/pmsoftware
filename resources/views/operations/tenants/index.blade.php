...
@section('title')
    All Tenants | {{ config('app.name', 'PM Software') }}
@endsection
        ...
        <div class="ps-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 p-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">ALL TENANTS ({{ $tenants->count() }})</li>
                </ol>
            </nav>
        </div>
        <div class="ms-auto">
                ...
                <table id="example" class="table align-middle table-hover table-striped table-bordered" style="width:100%">
                    ...
                    <tbody>
                        @foreach ($tenants as $index => $tenant)
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td><a class="text-dark" title="View" href="{{ route('tenants.show', $tenant->id) }}">{{ $tenant->name }}</a></td>
                                <td><a class="text-dark" title="View" href="{{ route('tenants.show', $tenant->id) }}">{{ $tenant->idorpassport }}</a></td>
